...
export function createSettings(parent:HTMLElement,onOpen?:()=>void,returnToMenu?:()=>void):()=>void {
 ...
 const escape=(event:KeyboardEvent)=>{if(event.code!=='Escape')return;event.preventDefault();event.stopImmediatePropagation();if(event.repeat)return;const fitting=document.querySelector<HTMLDialogElement>('.w2-dialog[open]');if(fitting){fitting.close();return;}if(dialog.open)close();else{document.querySelectorAll<HTMLDialogElement>('dialog[open]').forEach(d=>d.close());open();}};
