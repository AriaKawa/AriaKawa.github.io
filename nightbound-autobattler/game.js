(function () {
  ...
  const STORAGE_KEY = "nightboundAutobattler.records.v1";
  const bossSchedule = [
  ...
  function loadRecords() {
    try {
      ...
      return { bestTime: 0, bestKills: 0, bestLevel: 1, wins: 0, runs: 0, money: 0, lifetimeKills: 0, shop: {}, codex: [] };
    }
  }
  ...
  function saveRecords(won) {
    ...
    const next = {
      ...
      runs: (state.records.runs || 0) + 1,
      money: state.records.money || 0,
      lifetimeKills: state.records.lifetimeKills || 0,
      shop: Object.assign({}, state.records.shop || {}),
  ...
  function resetRun() {
    ...
    state.xpNeeded = 8;
    state.runMoney = 0;
    state.autoChoiceTimer = 0;
    state.shield = shopLevel("aegis") > 0 ? 1 : 0;
    state.player = {
      x: WORLD.width / 2,
      y: WORLD.height / 2,
      radius: 17,
      speed: 225 + shopLevel("speed") * 10,
      health: 100 + shopLevel("health") * 14,
      maxHealth: 100 + shopLevel("health") * 14,
      regen: 0.55 + shopLevel("regen") * 0.22,
      pickup: 86 + shopLevel("magnet") * 22,
      hurtTimer: 0,
      angle: 0
    };
    state.weapons = {
    ...
    logEvent(`Run ${state.records.runs + 1}: AI entered the courtyard.`);
    spawnBurst(18, "thrall");
    updateHud();
  }

  function shopLevel(id) {
    return Number((state.records.shop || {})[id] || 0);
  }

  function shopCost(item) {
    return Math.round(item.base * Math.pow(item.scale, shopLevel(item.id)));
  }

  function applyStartingItems() {
    if (shopLevel("startSigil") > 0) {
      state.weapons.sigil.level = Math.max(1, state.weapons.sigil.level);
      state.weapons.sigil.cooldown = 0.8;
    }
    if (shopLevel("startBats") > 0) {
      state.weapons.bats.level = Math.max(1, state.weapons.bats.level);
      state.weapons.bats.cooldown = 0.45;
    }
    if (shopLevel("extraBolt") > 0) {
      state.weapons.crossbow.count += 1;
      state.weapons.crossbow.damage += 8;
    }
  }

  function applyDamageMeta() {
    const multiplier = 1 + shopLevel("damage") * 0.08;
  ...
  function movementVector() {
    ...
    ui.strategy.textContent = gem && closeThreats < 8
      ? "AI pathing: scoop gems, kite the swarm, then snowball."
      : "AI pathing: danger high, orbiting away from pressure.";
    return { x: x / length, y: y / length, moving };
  }

  function bestGemTarget() {
    let best = null;
    let bestScore = -Infinity;
    state.gems.forEach((gem) => {
      const d = distance(state.player, gem);
      const score = gem.value * 18 - d;
      if (score > bestScore) {
        best = gem;
        bestScore = score;
      }
  ...
  function hurtPlayer(amount) {
    ...
    if (state.shield > 0) {
      ...
      addFloater(player.x, player.y - 24, "Aegis", "#77ff9b");
      return;
    }
  ...
  function killEnemy(enemy) {
    ...
    const earned = earnMoney(enemy);
    state.runMoney += earned;
    state.records.money = (state.records.money || 0) + earned;
    state.records.lifetimeKills = (state.records.lifetimeKills || 0) + (enemy.boss ? 8 : 1);
    saveMeta();
    addFloater(enemy.x, enemy.y + enemy.radius + 16, `$${earned}`, "#ffd166");
  ...
  function earnMoney(enemy) {
    const base = enemy.boss ? 45 : enemy.type.xp + 2;
    return Math.max(1, Math.round(base * (1 + shopLevel("income") * 0.1)));
  }
  ...
  function applyUpgradeChoice(choice) {
    choice.apply();
    logEvent(`Drafted ${choice.title} (${choice.kind}).`);
    state.mode = "playing";
    ui.upgradeOverlay.hidden = true;
    updateLoadout();
  }

  function updateAutoDraft(dt) {
    if (state.mode !== "upgrade") return;
    state.autoChoiceTimer -= dt;
    if (state.autoChoiceTimer > 0) return;
    const choice = state.pendingChoices
      .slice()
      .sort((a, b) => scoreUpgrade(b) - scoreUpgrade(a))[0];
    if (choice) applyUpgradeChoice(choice);
  }

  function scoreUpgrade(choice) {
    const healthRatio = state.player.health / state.player.maxHealth;
    const title = choice.title;
    let score = 0;
    if (title === "Moonshot Crossbow") score += 96 + state.elapsed / 18;
    if (title === "Blood Orbit") score += state.enemies.length > 34 ? 110 : 78;
    if (title === "Gravefire Sigil") score += state.weapons.sigil.level ? 88 : 104;
    if (title === "Bat Swarm") score += state.weapons.bats.level ? 82 : 98;
    if (title === "Crimson Heart") score += healthRatio < 0.55 ? 125 : 48;
    if (title === "Velvet Thirst") score += healthRatio < 0.72 ? 76 : 42;
    if (title === "Magnet Moon") score += state.gems.length > 18 ? 92 : 45;
    if (title === "Silver Boots") score += state.elapsed > 180 ? 88 : 54;
    score += Math.random() * 8;
    return score;
  }
  ...
  function endRun(won) {
    ...
    ui.endBody.textContent = won
      ? `The build reached dawn, banked $${state.runMoney}, bought ${buys} upgrade${buys === 1 ? "" : "s"}, and will loop again.`
      : `The AI banked $${state.runMoney}, bought ${buys} upgrade${buys === 1 ? "" : "s"}, and will restart automatically.`;
  ...
  function updateRecords() {
    ...
    ui.wins.textContent = `${state.records.wins || 0} wins / ${state.records.runs || 0} runs`;
  }

  function updateShop() {
    ui.money.textContent = `$${state.records.money || 0}`;
    ui.lifetimeKills.textContent = (state.records.lifetimeKills || 0).toString();
    ui.shopList.innerHTML = "";
    shopCatalog.forEach((item) => {
      const level = shopLevel(item.id);
      const done = level >= item.max;
      const row = document.createElement("div");
      row.className = `shop-item${done ? " is-bought" : ""}`;
      row.innerHTML = `
        <div>
          <span>${item.type} ${done ? "max" : `rank ${level}/${item.max}`}</span>
          <strong>${item.name}</strong>
          <span>${item.text}</span>
        </div>
        <b>${done ? "Owned" : `$${shopCost(item)}`}</b>
      `;
  ...
  function autoBuyShop() {
    ...
    for (let guard = 0; guard < 12; guard += 1) {
      const affordable = shopCatalog
        .filter((item) => shopLevel(item.id) < item.max && shopCost(item) <= (state.records.money || 0))
        .sort((a, b) => shopPriority(b) - shopPriority(a));
      if (!affordable.length) break;
      const item = affordable[0];
      state.records.money -= shopCost(item);
      state.records.shop[item.id] = shopLevel(item.id) + 1;
      buys += 1;
      logEvent(`Auto-shop bought ${item.name}.`);
    }
    saveMeta();
    return buys;
  }

  function shopPriority(item) {
    const level = shopLevel(item.id);
    const pressure = state.elapsed / Math.max(1, RUN_DURATION);
    const scores = {
      startSigil: level ? 0 : 140,
      startBats: level ? 0 : 132,
      damage: 118 + pressure * 34 - level * 2,
      health: state.player && state.player.health < state.player.maxHealth * 0.55 ? 130 : 90 - level,
      income: 112 - level * 4,
      extraBolt: level ? 0 : 104,
      speed: 76 + pressure * 28,
      magnet: state.level < 8 ? 84 : 68,
      regen: 74,
      aegis: level ? 0 : 82
    };
    return (scores[item.id] || 0) - shopCost(item) / 22;
  }

  function logEvent(text) {
    const line = document.createElement("p");
    line.innerHTML = `<span>${formatTime(state.elapsed)}</span>${text}`;
    ui.upgradeLog.prepend(line);
    while (ui.upgradeLog.children.length > 7) {
      ui.upgradeLog.lastElementChild.remove();
    }
  }
