...
export const floodForMap=(map:string)=>map==='forge' ? {grace:18,base:14,acceleration:.035,max:24} : map==='forest' ? {grace:10,base:30,acceleration:.08,max:52} : map==='magical' ? {grace:12,base:9,acceleration:.008,max:18} : map==='mountain'
