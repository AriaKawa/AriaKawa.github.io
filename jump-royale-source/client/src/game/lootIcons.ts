...
export function drawLootIcon(scene:Phaser.Scene,canvas:HTMLCanvasElement,item:LootEntry):void {
 if(item.slot==='animalHat'){drawAnimalHatIcon(canvas);return;}
 const sample=item.slot==='costume'?{...DEFAULT_OUTFIT,...costumeFields(item.id)}:item.slot==='hair'?{...DEFAULT_OUTFIT,character:item.id==='star-buns'?'magical-girl':'demon',hair:item.id}:{...DEFAULT_OUTFIT,character:item.id};
