...
export const LOOT_POOL:LootEntry[]=[
 ...
 ...MAGICAL_HAIR.filter(p=>p.id!=='original').map(p=>({slot:'hair',id:p.id,name:p.name,rarity:2})),
 ...ANIMAL_HATS.filter(p=>p.id!=='none').map(p=>({slot:'animalHat',id:p.id,name:p.name,rarity:p.id==='fedora'?1:2})),
