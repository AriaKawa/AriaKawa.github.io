...
assert(await p.locator('script[src*="index-CV6TPeuF"]').count());await p.getByRole('heading',{name:'The Long Mountain',exact:true}).waitFor();
await p.getByRole('button',{name:'Open wardrobe',exact:true}).click();assert.deepEqual(await p.getByRole('tab').allTextContents(),['Character','Costumes']);await p.getByRole('button',{name:'Close wardrobe',exact:true}).click();
const metrics=await p.evaluate(async()=>fetch(new URL('assets/jump-royale-ai/metrics.json?caps=2',location.href)).then(r=>r.json()));assert.equal(Object.keys(metrics).length,40);for(const m of Object.values(metrics))assert(m.capWidth>=m.width*.65&&m.capWidth<=m.width&&m.capLeft>=0&&m.capLeft+m.capWidth<=m.width);
...
console.log('PASS published bundle, costume tabs, forty valid platform caps, Foothills match, keyboard play, no dev global, zero request/runtime failures:',p.url());
