...
export class ResultsScene extends Phaser.Scene {
  ...
  create(data:{name:string;winner:string;placements:Placement[];localId:string;assisted?:boolean;goldEarned?:number;mapId?:MapId}):void {
    ...
    ui.querySelector('h1')!.textContent=(preferences.names?data.winner:'A climber')+' wins!';
    for(const p of data.placements??[]) {
      ...
      const earned=data.assisted?0:p.id===data.localId?(data.goldEarned??0):goldForPlace(p.place);
    ...
    ui.querySelector('footer p')!.textContent=data.assisted?'God powers round · personal records unchanged':'';
