...
export function solve(player:PlayerState,level:Platform[],time:number,target:Platform){
 ...
 for(const ticks of [24,23,22,21,20,19,18,17,16,15,14,12,10,8]){
  ...
  if(!frames)continue;
  for(const direction of [Math.sign(target.x+target.w/2-player.x)||1,0,-Math.sign(target.x+target.w/2-player.x)||-1])for(const offset of [0,-24,24,-48,48]){
   const desired=Math.max(source.x+12,Math.min(source.x+source.w-26,target.x+target.w/2-7-direction*350*frames/30+offset));
