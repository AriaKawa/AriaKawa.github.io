  ...
  update(zoom: number, world: SceneryWorld, sectors: WarSector[] = [], pressureSignature = ''): void {
    ...
    if(sig!==this.signature) {
      ...
      for(const p of props) {
        ...
        if(propRandom(p.id,733)<(biome==='green'?.12:biome==='autumn'?.18:.1)) put(`${p.id}:ground`,p.x+(propRandom(p.id,919)-.5)*p.size*.55,p.y+p.size*.3,'scenery-details',frame,p.size*(.5+propRandom(p.id,811)*.35),.38);
        if(biome==='dead' && p.frame===15 && propRandom(p.id,577)<.2) put(`${p.id}:embers`,p.x+p.size*.24,p.y+p.size*.25,'scenery-details',3,p.size*.65,1.36);
      }
