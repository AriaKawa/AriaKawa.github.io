...
try{
 ...
 for(const id of ['cerberus','magical-girl','skeleton','neet','kangaroo']){
  await page.getByRole('button',{name:'Open wardrobe',exact:true}).click();await page.getByRole('tab',{name:'Character',exact:true}).click();
  assert.equal(await page.locator('.wardrobe-overlay [data-piece]').count(),11);
 ...
 for(const name of ['Cerberus · Hellhound','Stella · Magical Girl','Rattle · Skeleton','Starlight Star Buns','Kenji · NEET','Roo · Kangaroo'])assert.equal(await page.locator('.loot-grid').getByText(name,{exact:true}).count(),1);
 ...
 const images=await page.evaluate(async()=>{const base=new URL('.',location.href);const list=[];for(const id of ['cerberus','magical-girl','magical-girl-buns','skeleton','neet','kangaroo']){const img=new Image();img.src=new URL(`assets/reforged/cosmetics/${id}.png`,base).href;await img.decode();list.push([img.naturalWidth,img.naturalHeight]);}return list;});
 assert.deepEqual(images,Array.from({length:6},()=>[384,32]));assert.deepEqual(errors,[]);
 console.log('PASS '+url+': 11-character wardrobe, new character purchases and persistence, two magical hairstyles, mystery-box entries and paid spin, all six sprite sheets, desktop/mobile, no browser errors.');
