...
type InternalZombie = ZombieState & { routePoints: Vec2[]; home?: Vec2; chasing?: boolean; nextAttackAt?: number };
...
const SATCHEL_DROP_CHANCE: Record<ZombieState["type"], number> = { shambler: .02, runner: .02, bloater: .08, armored: .09, brute: .22, walker: .01, crawler: .02, stalker: .05, skitter: .05, ravager: .13, zombieKing: 1 };
...
export class LocalSimulation {
  ...
  private campaignDead = false;
  private pausedAt = 0;
  setPaused(paused: boolean): void {
    if(paused) { if(!this.pausedAt) this.pausedAt=Date.now(); this.saveCharacter(); return; }
    if(!this.pausedAt) return;
    const delta=Date.now()-this.pausedAt; this.pausedAt=0;
    const shift=(obj:any):void=>{if(!obj || typeof obj!=='object')return; for(const key of Object.keys(obj)){const value=obj[key];if(typeof value==='number' && /At$|Until$|EndsAt$/.test(key) && value>1e12 && value<1e14)obj[key]+=delta;else if(typeof value==='object')shift(value);}};
    for(const value of [this.player,this.bases,this.contracts,this.zombies,this.towers,this.placedEquipment,this.lootSatchels])shift(value);
  }
  ...
  private ambientDefeated = new Map<string,number>();
  private nextAmbientScan = 0;
  private worldDeaths: Array<{id:string;x:number;y:number;at:number}> = [];
  ...
  dispose(): void {
    this.saveCharacter(); window.removeEventListener?.("pagehide", this.onPageHide);
  ...
  constructor(name: string, private snapshotListener: (snapshot: Snapshot) => void, private noticeListener: (notice: Notice) => void, private managed = false) {
    ...
    if (!managed) {
      ...
      if (character?.status === 'alive') {
        ...
        if (run?.player) {
          const elapsed = Math.max(0, Date.now() - (character.savedAt || Date.now()));
          const shift = (value: any): any => Array.isArray(value) ? value.map(shift) : value && typeof value === 'object' ? Object.fromEntries(Object.entries(value).map(([key,v]) => [key, typeof v === 'number' && /At$|Until$|EndsAt$/.test(key) && v > 1e12 && v < 1e14 ? v + elapsed : shift(v)])) : value;
          const restored = shift(run);
          this.player = { ...this.player, ...restored.player, id: this.playerId, name: character.name };
          for (const key of ['territories','lots','bases','towers','zombies','contracts','warSectors','warOperations','squadCommanders','lootSatchels','placedEquipment','activeStrategicContext','id','radio','activityFeed'] as const) if (restored[key] !== undefined) (this as any)[key] = restored[key];
          if (restored.roads) this.world.roads = restored.roads;
        } else this.player.equipmentStash = this.hideoutLevels.field ? [{ type: 'fieldRepairKit', count: this.hideoutLevels.field }] : [];
      }
      if (!this.bases.length && this.hideoutLevels.field) {
  ...
  saveCharacter(): void {
    if (this.managed || !this.characterId || this.campaignDead) return;
    try {
      const campaign = loadCampaign(); const character = campaign.slots.find(s => s?.id === this.characterId);
      if (!character || character.status !== 'alive') return;
      character.xp = this.player.xp || 0; character.scrap = this.player.scrap; character.savedAt = this.pausedAt || Date.now();
      const run: any = { player: this.player, roads: this.world.roads };
      for (const key of ['territories','lots','bases','towers','zombies','contracts','warSectors','warOperations','squadCommanders','lootSatchels','placedEquipment','activeStrategicContext','id','radio','activityFeed'] as const) run[key] = this[key];
      character.run = run; saveCampaign(campaign); this.lastCampaignSave = Date.now();
    } catch { this.notice('Save failed: browser storage is full or unavailable. Free space before leaving.', true); }
  }
  private saveProgression(): void { this.saveCharacter(); }
  private loseCharacter(): void {
    if (this.managed || !this.characterId || this.campaignDead) return;
    settleCharacter(loadCampaign(), this.characterId, this.player.xp || 0, this.player.scrap, true);
    this.campaignDead = true; window.clearInterval(this.tickTimer);
    window.dispatchEvent(new CustomEvent('deadroad-character-died'));
  }
  extractToHideout(): boolean {
    if (this.campaignDead) return false;
    const base = this.bases.find(b => b.id === this.player.baseId);
  ...
  private deploymentDifficulty(base: BaseState): number {
    const origin = { x: base.worldX ?? this.activeStrategicContext?.worldX ?? 0, y: base.worldY ?? this.activeStrategicContext?.worldY ?? 0 };
    const ll = theaterLatLon({ x: origin.x + (base.coreX - (base.anchorX ?? base.coreX)) * TACTICAL_MAP_SCALE, y: origin.y + (base.coreY - (base.anchorY ?? base.coreY)) * TACTICAL_MAP_SCALE });
    const threat = base.worldX !== undefined ? threatAt(ll.lat, ll.lon, this.warSectors) : this.territories.find(t => t.id === base.territoryId)?.threatLevel ?? 1;
    return missionDifficulty(threat, this.player.xp);
  }
  private routeVariant = 0;
  private previewDifficulty?: number;
  private routeSite = "";
  previewDeployment(reroll = false): { difficulty: number; rewards: ReturnType<typeof missionRewards>; routes: Vec2[][]; core: Vec2; stamp: string } | undefined {
    const base = this.bases.find(b => b.id === this.player.baseId); if (!base || base.status !== 'packed') return;
    const source = this.lots.find(l => l.id === base.lotId); if (!source) return;
    const site = `${base.id}:${base.coreX.toFixed(0)}:${base.coreY.toFixed(0)}`; if (site !== this.routeSite) { this.routeSite = site; this.routeVariant = 0; this.previewDifficulty = undefined; }
    if (reroll) { const previous = this.previewDeployment(); const variant = this.routeVariant;
      for (let attempt = 1; attempt <= 8; attempt++) { this.routeVariant = variant + attempt; const next = this.previewDeployment(); if (next && JSON.stringify(next.routes) !== JSON.stringify(previous?.routes)) return next; }
      this.routeVariant = variant; this.notice('No other clear approach was found. Keeping your current route.'); return previous;
    }
    const target = structuredClone(source); if (!placeLotAtConvoy(target, base, this.world)) return;
    const difficulty = this.previewDifficulty ??= this.deploymentDifficulty(base); const contractId = 'preview';
    const seed = defenseContractSeed(`${target.id}:${base.coreX.toFixed(0)}:${base.coreY.toFixed(0)}:${this.routeVariant}`);
    try { const layout = generateDefenseContractLayout(target, difficulty, contractId, seed, (points, lane) => this.scenery ? this.scenery.deploymentRoute(points.map(this.projectScenery), 22*this.sceneryScale, this.sceneryScale, this.routeVariant, lane > 0).map(this.unprojectScenery) : points);
      return { difficulty, rewards: missionRewards(difficulty), routes: layout.routes.map(r => r.points), core: { x: base.coreX, y: base.coreY }, stamp: `${base.id}:${base.coreX}:${base.coreY}:${difficulty}:${this.routeVariant}` };
    } catch { return; }
  }
  deployConvoy(strategicOnLand?: boolean, previewStamp?: string): void {
    if (previewStamp && this.previewDeployment()?.stamp !== previewStamp) return this.notice('Deployment changed. Preview the new site before confirming.', true);
    ...
    const now = Date.now(); const contractId = `contract-${this.id++}`; const difficulty = previewStamp ? this.previewDifficulty ?? this.deploymentDifficulty(base) : this.deploymentDifficulty(base); const seed = defenseContractSeed(`${target.id}:${base.coreX.toFixed(0)}:${base.coreY.toFixed(0)}:${this.routeVariant}`); let layout: ReturnType<typeof generateDefenseContractLayout>;
    try { layout = generateDefenseContractLayout(target, difficulty, contractId, seed, (points, lane) => this.scenery ? this.scenery.deploymentRoute(points.map(this.projectScenery), 22*this.sceneryScale, this.sceneryScale, this.routeVariant, lane > 0).map(this.unprojectScenery) : points); }
    ...
    if (base.worldX !== undefined && base.worldY !== undefined) {
      const ll=theaterLatLon({x:base.worldX+(base.coreX-(base.anchorX??base.coreX))*TACTICAL_MAP_SCALE,y:base.worldY+(base.coreY-(base.anchorY??base.coreY))*TACTICAL_MAP_SCALE}); const region=earthTerritoryAt(ll.lat,ll.lon);
      if(region) { const sector=this.ensureStrategicSector({planetId:'earth',regionId:region.id,regionName:region.name,centerLat:ll.lat,centerLon:ll.lon,threatLevel:region.threat,infestation:region.infestation,state:region.infestation>=72?'infested':'contested'}); operation.sectorId=sector.id; operation.visibleLat=ll.lat; operation.visibleLon=ll.lon; }
    }
    ...
    this.contracts.push({ deployedAt: now, id: contractId, ownerPlayerId: this.playerId, baseId: base.id, sectorId: operation.sectorId, territoryId: target.territoryId, lotId: target.id, basePosition: { x: base.coreX, y: base.coreY }, corePosition: { x: base.coreX, y: base.coreY }, spawnpoints: layout.spawnpoints, routes: layout.routes, buildPads: layout.buildPads, spawnPointId: primarySpawn.id, routePoints: primaryRoute.points, nextWaveAt: now + 31200, nextSpawnAt: 0, waveIndex: 0, difficulty, threatLevel: difficulty, routeDanger: layout.routeDanger, status: "setup", queue: [], activeRouteIds: layout.routes.map(route => route.id) });
  ...
  upgrade(towerId: string, path: ResearchPath = "power"): void {
    const tower = this.towers.find((entry) => entry.id === towerId && entry.ownerPlayerId === this.playerId); if (!tower || tower.packed || !RESEARCH_PATHS.includes(path) || tower.type === "squadBeacon" && tower.level >= 3) return;
    ...
    const tier = (tower.pathTiers?.[path] ?? 0) + 1;
    if (tier > 3 || (this.player.research?.[`${tower.type}:${path}`] ?? 0) < tier) return this.notice('Research this branch in the armory first.', true);
    const tiers = tower.pathTiers ?? {}; const others = RESEARCH_PATHS.filter(p => p !== path && (tiers[p] ?? 0) > 0);
    if (others.length >= 2 || tier > 1 && others.some(p => (tiers[p] ?? 0) > 1)) return this.notice('Choose one main branch and one tier-1 support branch per turret.', true);
    const cost = COMBAT_PROGRESSION.towerUpgradeCostBase * tier; if (this.player.scrap < cost) return this.notice('Not enough scrap for that upgrade.', true);
    this.player.scrap -= cost; (tower.pathTiers ??= {})[path] = tier; tower.level = Math.min(3, 1 + Math.max(...Object.values(tower.pathTiers))); applyResearchUpgrade(tower, path); this.emit();
  }
  ...
  driveBase(steering: number, throttle: number, deltaMs: number, onVisibleHighway = false): void {
    ...
    const rawHeading = base.heading ?? 0; const heading = previousHeading + Math.atan2(Math.sin(rawHeading - previousHeading), Math.cos(rawHeading - previousHeading)); const nextX = base.coreX + Math.cos(heading) * distance; const nextY = base.coreY + Math.sin(heading) * distance;
    ...
    if(Math.hypot(safe.x-start.x,safe.y-start.y)>.01) {
      for(const zombie of [...this.zombies]) {
        // Sweep the chassis so fast travel and reversing cannot skip a zombie.
        const hit=[-20,-10,0,10,20].some(offset=>distanceToSegment(zombie,
          {x:start.x+Math.cos(previousHeading)*offset,y:start.y+Math.sin(previousHeading)*offset},
          {x:safe.x+Math.cos(base.heading!)*offset,y:safe.y+Math.sin(base.heading!)*offset})<=24);
        if(hit) {
          if(!zombie.home) this.worldDeaths.push({id:zombie.id,x:zombie.x,y:zombie.y,at:Date.now()});
          this.damage(zombie,zombie.hp);
        }
      }
    }
  ...
  private tick(): void {
    ...
    if (this.campaignDead) return;
    this.updateAmbientPopulation(now);
    this.worldDeaths=this.worldDeaths.filter(d=>now-d.at<2000);
  ...
  private moveAmbient(zombie: InternalZombie, dt: number, now: number): void {
    ...
    if(zombie.chasing && d<42) {
      if(now>=(zombie.nextAttackAt??0) && now>=base.shieldEndsAt) {
        zombie.nextAttackAt=now+1000; base.hp-=ZOMBIE_INFO[zombie.type].damage;
        if(base.hp<=0) {this.notice('Your convoy was overrun.',true);this.loseCharacter();this.removeBase(false);}
      }
      return;
    }
    if(d>3) {const step=Math.min(d,zombie.speed*dt),safe=this.sceneryMove(zombie,{x:zombie.x+dx/d*step,y:zombie.y+dy/d*step},18);zombie.x=safe.x;zombie.y=safe.y;}
  }
  private startWave(contract: DefenseContract, now: number): void {
    contract.waveIndex += 1; const original = planZombieWave(contract.waveIndex); const roster = [...original, ...Array.from({ length: Math.floor(original.length * (enemyCountMultiplier(contract.difficulty) - 1)) }, (_, i) => original[i % original.length]).filter(type => type !== "zombieKing" && type !== "brute")]; const activeRoutes = contract.routes; contract.activeRouteIds = activeRoutes.map((entry) => entry.id);
  ...
  private spawn(contract: DefenseContract, order: DefenseContract["queue"][number]): void {
    const route = contract.routes.find((entry) => entry.id === order.routeId) ?? contract.routes[0]; const stats = ZOMBIE_INFO[order.zombieType];  const scale = waveHealthScale(contract.waveIndex) * enemyHealthMultiplier(contract.difficulty); const leaveRoadAt = Math.max(2, route.points.length - 3); const rawRoutePoints = order.roadFollowing || route.roadIds.length === 0 ? route.points : [...route.points.slice(0, leaveRoadAt), contract.corePosition];
  ...
  private updateTowers(now: number): void {
    for (const tower of this.towers.filter((entry) => entry.ownerPlayerId === this.playerId && !entry.packed)) {
      ...
      if (tower.type === "cannon") targets.slice(1).filter((entry) => Math.hypot(entry.x - target.x, entry.y - target.y) < 65).forEach((entry) => this.damage(entry, effectiveDamage * .5));
  ...
  private moveZombie(zombie: InternalZombie, dt: number, now: number): void {
    ...
    if(zombie.home) {this.moveAmbient(zombie,dt,now);return;}
  ...
  private hitCore(zombie: InternalZombie): void {
    ...
    base.hp -= ZOMBIE_INFO[zombie.type].damage; base.status = "underAttack"; if (base.hp <= 0) { const operation = this.warOperations.find((entry) => entry.id === base.operationId); const sector = this.warSectors.find((entry) => entry.id === operation?.sectorId); if (operation) { operation.status = "failed"; operation.casualties += 1; operation.updatedAt = Date.now(); } if (sector) { sector.pressure = Math.min(100, sector.pressure + 16); sector.supply = Math.max(0, sector.supply - 8); sector.lastEventText = `${this.player.name}'s core was lost.`; } this.radio = `Command core fell in ${sector?.name ?? "the sector"}. Zombie pressure surged.`; this.pushFeed(this.radio); this.syncSectorState(); this.loseCharacter(); this.removeBase(false); }
  }

  private finishWave(contract: DefenseContract, now: number): void {
    const base = this.bases.find((entry) => entry.id === contract.baseId); const scavengerBonus = Math.min(.2, this.activeSquad(base?.id, "scavenger").length * .1); contract.nextWaveAt = now + waveRestMs(contract.waveIndex); const reward = missionRewards(contract.difficulty, contract.waveIndex); this.player.scrap += Math.round(reward.scrap * (1 + scavengerBonus)); awardXp(this.player, reward.xp);
    if (Math.random() < reward.turretChance) { const types = ['rifle', 'cannon', 'flame', 'shock', 'floodlight']; const type = types[Math.floor(Math.random() * types.length)]; (this.player.turretRewards ??= []).push({ type, path: RESEARCH_PATHS[Math.floor(Math.random() * 3)], tier: reward.turretTier }); this.notice(`Salvaged a free tier-${reward.turretTier} ${type} turret. Build it from the fabricator.`); } this.saveProgression(); this.player.signal += 1; const operation = this.warOperations.find((entry) => entry.id === base?.operationId); const sector = this.warSectors.find((entry) => entry.id === operation?.sectorId); const signalBonus = Math.min(.3, this.activeSquad(base?.id, "signal").length * .15); const reduction = Math.round((base?.kind === "reclaim" ? 16 : 5) * (1 + signalBonus) * 10) / 10;
