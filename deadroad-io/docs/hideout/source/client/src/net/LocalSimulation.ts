...
export class LocalSimulation {
  ...
  previewDeployment(reroll = false): { difficulty: number; rewards: ReturnType<typeof missionRewards>; routes: Vec2[][]; core: Vec2; stamp: string } | undefined {
    ...
    try { const layout = generateDefenseContractLayout(target, difficulty, contractId, seed, points => this.scenery ? this.scenery.deploymentRoute(points.map(this.projectScenery), 22*this.sceneryScale, this.sceneryScale, this.routeVariant).map(this.unprojectScenery) : points);
  ...
  deployConvoy(strategicOnLand?: boolean, previewStamp?: string): void {
    ...
    try { layout = generateDefenseContractLayout(target, difficulty, contractId, seed, points => this.scenery ? this.scenery.deploymentRoute(points.map(this.projectScenery), 22*this.sceneryScale, this.sceneryScale, this.routeVariant).map(this.unprojectScenery) : points); }
