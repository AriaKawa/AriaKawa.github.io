...
function read():Preferences {try{const p=JSON.parse(localStorage.getItem(KEY)||'{}');return {music:clamp(p.music,.3),effects:clamp(p.effects,1),enemyOpacity:clamp(p.enemyOpacity,BOT_ALPHA),names:typeof p.names==='boolean'?p.names:true};}catch{return {music:.3,effects:1,enemyOpacity:BOT_ALPHA,names:true};}}
...
export function createSettings(parent:HTMLElement,onOpen?:()=>void,returnToMenu?:()=>void):()=>void {
 ...
 dialog.innerHTML='<header><h2>Settings</h2><button type="button" class="pixel-button" aria-label="Close settings"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6L18 18M18 6L6 18" fill="none" stroke="currentColor" stroke-width="2"/></svg></button></header><label>Music <output></output><input type="range" min="0" max="100" step="1" aria-label="Music volume"></label><label>Sound effects <output></output><input type="range" min="0" max="100" step="1" aria-label="Sound effects volume"></label><label>Enemy opacity <output></output><input type="range" min="0" max="100" step="1" aria-label="Enemy opacity"></label><label class="names-setting"><span>Show player names</span><input type="checkbox" aria-label="Show player names"></label>';
