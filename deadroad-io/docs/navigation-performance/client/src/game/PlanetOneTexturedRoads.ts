...
import { roadRasterCovers, roadRasterView, type RoadRasterView, type RoadRasterResult } from "./PlanetOneRoadRaster";
...
export class PlanetOneTexturedRoads {
  ...
  get ready(): boolean {
    const r = this.raster, v = this.view;
    return !!r && !!v && !this.failed && v.left >= r.left && v.top >= r.top
      && v.left + v.width / v.zoom <= r.left + r.width / r.zoom
      && v.top + v.height / v.zoom <= r.top + r.height / r.zoom;
  }
  ...
  refresh(camera: Phaser.Cameras.Scene2D.Camera, zoom: number): void {
    ...
    if (this.pending || (this.raster && roadRasterCovers(this.raster, view))) return;
