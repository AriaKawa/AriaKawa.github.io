...
The precision revision keeps the broad catch galleries and raises the difficulty between them: most mid/upper ledges are 18–32px wide, with committed horizontal jumps typically 165–205px. The first five foothill ledges taper from 110px to 52px. Moving route platforms retain 56px caps and ice 96px so the existing momentum and grip mechanics remain usable. Every placement is explicitly authored and collision-tested.
...
- Two full 281-jump simulation replays at different moving-platform/wind phases passed through actual walking, charging and collision integration. Minimum full-body landing margin: 0.4px across both phase runs.
- 584 fall probes recovered onto geometry; observed drops span 0–6,005px. All broad recovery platforms have tested exits back into the route.
