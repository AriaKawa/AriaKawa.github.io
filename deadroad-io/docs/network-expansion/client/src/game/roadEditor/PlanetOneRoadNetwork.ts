...
import { expandUSARoads } from "./ExpandedUSARoads";
...
export function planetOneAuthoredRoads(includeDirt = true) {
  ...
  return expandUSARoads(cloneRoads(roads), includeDirt);
}
...
export function planetOneWorldRoads(): WorldRoad[] {
  return planetOneAuthoredRoads().map(r => ({
    id: r.id, name: r.name, roadClass: r.roadClass === "highway" ? "interstate_highway" : "county_road", gameRoadClass: r.roadClass === "highway" ? "interstate" : r.roadClass === "dirt_road" ? "dirt_road" : "rural_road",
    renderClass: r.roadClass === "highway" ? "highway" : r.roadClass === "dirt_road" ? "dirt" : "secondary", purpose: "connect_city", surface: r.roadClass === "dirt_road" ? "dirt" : "damaged_asphalt",
