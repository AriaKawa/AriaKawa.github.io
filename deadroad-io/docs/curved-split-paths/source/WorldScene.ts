import { portalPlacement, PORTAL_ROAD_WIDTH } from "../game/PortalPlacement";
...
import { paintThreatFieldAsync, threatAt } from '../game/ThreatField';
...
export class WorldScene extends Phaser.Scene {
  ...
  private lastSurfaceBakeAt = -Infinity;
  ...
  private seenWorldDeaths = new Set<string>();
  ...
  preload(): void {
    ...
    for (const biome of ["autumn", "dead", "details"]) this.load.image(`scenery-${biome}`, `./assets/generated/scenery-${biome}-v1.png`);
    ...
    this.load.image("zombie-spawn-gravestone", "./assets/generated/zombie_spawn_portal_topdown.png");
  }
  ...
  private revealWorldWhenReady(): void {
    ...
    this.worldRevealed = true; this.network.setPaused(false); if (status) status.textContent = "THEATER READY // OPENING COMMAND VIEW";
  ...
  update(_time: number, deltaMs: number): void {
    ...
    if (this.sceneryWorld) this.expeditionScenery?.update(zoom, this.sceneryWorld, this.snapshot?.warSectors, this.threatSignature);
    ...
    if (this.snapshot) {
      ...
      const deaths=this.snapshot.worldDeaths??[];
      for(const death of deaths) if(!this.seenWorldDeaths.has(death.id)) {
        this.spawnBloodDecal(death.id,{...death,angle:0,walkPhase:0,contractId:'ambient',type:'walker',speed:0});
        playBloodSplat(this.spatialAudioGainForSimulationPoint(death.x,death.y));
      }
      this.seenWorldDeaths=new Set(deaths.map(d=>d.id));
      const liveIds = new Set<string>();
      for (const zombie of this.snapshot.zombies) {
        const point = this.theaterWorld ? this.toTheater(zombie) : zombie;
        if ((this.effectiveZoom() < .055) || !this.pointInView(point.x, point.y, 650)) continue;
      ...
      for (const [id, render] of this.renderedZombies) if (!liveIds.has(id)) {
        ...
        const visible = (this.effectiveZoom() >= .055) && this.pointInView(point.x, point.y, 500);
  ...
  private async captureDeploymentMap(preview: DeploymentPreview): Promise<HTMLCanvasElement> {
    ...
    try {
      const coreWorld=projectWorld(preview.core);
      // A fixed site-centered survey keeps the convoy at the same screen position on every reroll.
      const extentX=Math.max(coreWorld.x-left,right-coreWorld.x)+margin;
      const extentY=Math.max(coreWorld.y-top,bottom-coreWorld.y)+margin;
      camera.setZoom(Math.min(cropWidth/(extentX*2),cropHeight/(extentY*2))); camera.centerOn(coreWorld.x,coreWorld.y);
  ...
  private threatRendering = false;
  private paintedThreatSignature = '';
  private async refreshThreatField(): Promise<void> {
    if (!this.theaterWorld || this.threatRendering || (this.threatImage && this.paintedThreatSignature === this.threatSignature)) return;
    this.threatRendering = true;
    const signature = this.threatSignature;
    const canvas = document.createElement('canvas'); canvas.width = 1024; canvas.height = 1408;
    const ctx = canvas.getContext('2d')!;
    const sectors = this.snapshot?.warSectors.map(s => ({ ...s })) ?? [];
    try {
      ctx.save(); ctx.beginPath();
      for (const mass of planetOneTacticalLandmasses()) { mass.points.forEach((p, i) => i ? ctx.lineTo(p.x / THEATER_WIDTH * 1024, p.y / THEATER_HEIGHT * 1408) : ctx.moveTo(p.x / THEATER_WIDTH * 1024, p.y / THEATER_HEIGHT * 1408)); ctx.closePath(); }
      ctx.clip();
      await paintThreatFieldAsync(ctx, 1024, 1408, (x, y) => theaterLatLon({ x: x / 1024 * THEATER_WIDTH, y: y / 1408 * THEATER_HEIGHT }), sectors, 4);
      ctx.restore();
      if (!this.sys.isActive()) return;
      const texture = this.threatImage ? this.textures.get('threat-field') as Phaser.Textures.CanvasTexture : this.textures.createCanvas('threat-field', 1024, 1408)!;
      texture.context.clearRect(0, 0, 1024, 1408); texture.context.drawImage(canvas, 0, 0); texture.refresh();
      this.threatImage ??= this.add.image(0, 0, 'threat-field').setOrigin(0).setDisplaySize(THEATER_WIDTH * BATTLE_WORLD_SCALE, THEATER_HEIGHT * BATTLE_WORLD_SCALE).setDepth(.25);
      this.paintedThreatSignature = signature;
    } finally { this.threatRendering = false; }
    if (this.sys.isActive() && signature !== this.threatSignature) void this.refreshThreatField();
  }
  ...
  private refreshSurfaceTexture(camera: Phaser.Cameras.Scene2D.Camera, zoom: number): void {
    ...
    const firstColumn = Math.max(0, Math.floor(view.left / chunkSize)); const lastColumn = Math.min(Math.ceil(THEATER_WIDTH / chunkSize) - 1, Math.floor(view.right / chunkSize)); const firstRow = Math.max(0, Math.floor(view.top / chunkSize)); const lastRow = Math.min(Math.ceil(THEATER_HEIGHT / chunkSize) - 1, Math.floor(view.bottom / chunkSize)); const signature = `${lod}:${firstColumn}:${lastColumn}:${firstRow}:${lastRow}:${Math.round(alpha * 20)}`; if (signature === this.lastSurfaceSignature) return; const cameraSignature = `${camera.worldView.left.toFixed(1)}:${camera.worldView.top.toFixed(1)}:${zoom.toFixed(5)}:${camera.width}:${camera.height}`; const now = performance.now(); if (cameraSignature !== this.surfaceCameraSignature) { this.surfaceCameraSignature = cameraSignature; this.surfaceCameraChangedAt = now; } const mayBake = now - this.surfaceCameraChangedAt >= 120 || now - this.lastSurfaceBakeAt >= 150; let pending = 0; const active = new Set<string>(); this.surfaceUseCounter++; const refreshStarted = performance.now(); let created = 0;
    // Fill the ground under the convoy first; continuous driving must not starve baking.
    const cells: Array<{row:number;column:number}> = [];
    for(let row=firstRow;row<=lastRow;row++) for(let column=firstColumn;column<=lastColumn;column++) cells.push({row,column});
    const centerColumn=(view.left+view.right)/2/chunkSize,centerRow=(view.top+view.bottom)/2/chunkSize;
    cells.sort((a,b)=>Math.hypot(a.column+.5-centerColumn,a.row+.5-centerRow)-Math.hypot(b.column+.5-centerColumn,b.row+.5-centerRow));
    for (const {row,column} of cells) {
      const cacheKey = `${lod}:${column}:${row}`; active.add(cacheKey); let chunk = this.surfaceChunks.get(cacheKey); if (!chunk && (!mayBake || created >= 1)) { pending++; continue; } if (!chunk) { chunk = this.createSurfaceChunk(cacheKey, column * chunkSize, row * chunkSize, chunkSize, pixels, lod); this.surfaceChunks.set(cacheKey, chunk); this.lastSurfaceBakeAt=performance.now(); created++; }
  ...
  private retireZombieVisual(id: string, render: ZombieVisualState, died = true): void {
    ...
    if (died && contract && !reachedCore && !this.seenWorldDeaths.has(id)) { playBloodSplat(this.spatialAudioGainForSimulationPoint(render.x, render.y)); this.spawnBloodDecal(id, render); }
  ...
  private updateZombieSprites(): void {
    const visibleAtBand = this.effectiveZoom() >= .055; let visible = 0;
  ...
  private drawDynamic(): void {
    ...
    if (zoom >= .055) for (const tower of this.snapshot.towers.filter((entry) => !entry.packed)) {
    ...
    if (zoom >= .055) for (const zombie of this.snapshot.zombies) {
    ...
    this.syncFieldEquipmentSprites(now, zoom >= .055);
  ...
  private drawRoutePreview(g: Phaser.GameObjects.Graphics, points: Vec2[], progress = 1): void { this.drawGeneratedDirtPath(g, points, PORTAL_ROAD_WIDTH, .96, true, progress); }
  ...
  private drawContractPreview(g: Phaser.GameObjects.Graphics, contract: DefenseContract | undefined, lot: RoadsideLot, now: number): void {
    ...
    for (const route of contractRoutes) {
      ...
      const spawn = contract?.spawnpoints?.find((entry) => entry.id === route.spawnpointId); if (spawn && contract) this.drawGravestoneSpawner(`${contract.id}:${route.id}`, { x: spawn.worldX, y: spawn.worldY }, points, now);
  ...
  private drawGravestoneSpawner(markerId: string, spawn: Vec2, routePoints: Vec2[], now: number): void {
    const point = this.theaterWorld ? this.toTheater(spawn) : spawn;
    const placement = portalPlacement(point, routePoints);
    let marker = this.spawnerMarkers.get(markerId);
    if (!marker) {
      marker = { image: this.add.image(0, 0, "zombie-spawn-gravestone").setOrigin(.5, .51).setDepth(ZOMBIE_SPRITE_DEPTH - .08), bornAt: now };
      this.spawnerMarkers.set(markerId, marker);
    }
    const progress = Phaser.Math.Clamp((now - marker.bornAt) / 720, 0, 1);
    // The transparent asset's inner pillar edges are at 29% and 71%.
    // Keep the opening road-wide throughout materialization; never billboard it.
    const size = PORTAL_ROAD_WIDTH / .42 * BATTLE_WORLD_SCALE;
    marker.image.setVisible(true).setPosition(point.x * BATTLE_WORLD_SCALE, point.y * BATTLE_WORLD_SCALE)
      .setDisplaySize(size, size).setRotation(placement.angle).setAlpha(progress);
    const fx = this.projectileEffectGraphics, pulse = .5 + .5 * Math.sin(now * .003);
    const cos = Math.cos(placement.angle), sin = Math.sin(placement.angle);
    const localPoint = (x: number, y: number) => ({ x: point.x + cos * x - sin * y, y: point.y + sin * x + cos * y });
    for (let i = 0; i < 12; i++) {
      const phase = now * .0016 + i * Math.PI * 2 / 12;
      const p = localPoint(Math.cos(phase) * PORTAL_ROAD_WIDTH * .46, Math.sin(phase * 2) * 3);
      fx.fillStyle(i % 2 ? 0x62ffe1 : 0xb57aff, (.35 + pulse * .35) * progress).fillCircle(p.x, p.y, i % 3 === 0 ? .8 : .5);
    }
  }
