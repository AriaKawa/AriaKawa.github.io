...
import { curatedUSARoads, USA_ANCHORS } from "../client/src/game/roadEditor/CuratedUSARoadLayer.ts";
...
for (const name of ["stLouis", "kc", "chicago", "la", "atlanta", "newYork"] as const) {
  ...
  assert(node && node.edges.length >= 3 && node.edges.length <= 6, `${name} must be one simple 3–6 arm junction`);
...
await writeFile(new URL("../docs/curated-usa/curated-usa-v1.json", import.meta.url), JSON.stringify(exported, null, 2));
console.log(JSON.stringify({roads: roads.length, controlPoints: roads.reduce((n,r)=>n+r.points.length,0), connected: true, onLand: true, metros, exportRoundTrip: true, planet1Unchanged: true}, null, 2));
