...
for(let tick=0;tick<9000;tick++){const now=tick*1000/30;updateMovingPlatforms(level,now);for(const p of bots){updateBot(p,level,now);stepPlayer(p,level,1/30);}}
