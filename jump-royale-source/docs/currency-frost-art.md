...
The icon sits immediately after the loot box. It is vertically centered with equal top and bottom spacing and links to https://buymeacoffee.com/ariakawa in a new tab, with an accessible label and keyboard focus indicator. The right-aligned toolbar shifts preceding items left, with a compact narrow-phone layout. Browser verification covers 104 outfit/hat/retro size combinations, four viewport widths, and forced high-danger HUD updates on every map with camera shake instrumented to fail the test.

## Zoom, opacity and grounded contact

Coffee and Settings now use matching framed utility boxes at each breakpoint. Enemy opacity ranges from 0% to 100% and defaults to the existing BOT_ALPHA value (25%), including when migrating earlier saved settings. The preference applies to other climbers, their names and charge bars; the local player remains visible. Animal visual scale is now 0.575 of normal body height, a 15% increase over the previous half-size presentation.

Gameplay uses 1.25 camera zoom. A separate unzoomed camera renders the HUD, and DOM world-label coordinates follow the main camera zoom. Grounded sprites use each current animation frame's opaque foot baseline and snap their vertical presentation to the shared collision surface. Airborne interpolation and identical per-character physics are preserved. Lobby grounded poses use the same frame-aware baseline.

Validation: all 856 platform center landing surfaces matched their visual art; 936 grounded outfit/hat/retro poses per map (5,616 across all six) matched the physics landing position exactly. Browser tests cover equal utility boxes at 320/390/768/1440px, opacity migration/default/persistence, live opacity endpoints, animal sizing, 1.25 world camera and 1.0 HUD camera. Simulation tests and production build pass.
