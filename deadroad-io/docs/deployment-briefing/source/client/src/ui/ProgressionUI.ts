...
export class ProgressionUI {
  ...
  async preview(confirm: (stamp: string) => void, reroll = false): Promise<boolean> {
    ...
    const briefing = document.createElement('p'); briefing.textContent = 'Approach route confirmed. Rewards are paid for each cleared wave; subsequent waves grow stronger. Your convoy remains packed until confirmation.'; this.dialog.append(briefing);
  ...
  armory(tower?: TowerState, selected: TowerType = tower?.type ?? 'rifle') {
    ...
    for (const path of RESEARCH_PATHS) { const unlocked = p?.research?.[`${selected}:${path}`] ?? 0; const installed = tower?.pathTiers?.[path] ?? 0; const card = document.createElement('section'); const title = document.createElement('h3'); title.textContent = PATH_LABELS[path]; card.append(title); const effect = document.createElement('p'); effect.textContent = path === 'power' ? '+45% damage per tier' : path === 'reach' ? '+23% range, +10% damage per tier' : '+30% fire rate per tier'; card.append(effect);
