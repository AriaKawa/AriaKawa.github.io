...
export class WorldScene extends Phaser.Scene {
  ...
  private deployConvoy(): void { const base = this.localBase(); if (!base || base.status !== "packed") return; if (!this.world) return; const strategicOnLand = this.convoyStrategicOnLand(base); const assessment = this.assessConvoySite(base, this.hordeActive(base)); if (!assessment.valid) { this.showNotice(assessment.message, true); return; } this.openHudPanel("none"); this.network.deployConvoy(strategicOnLand); }
  ...
  private dirtPathImages = new Map<string, {image: Phaser.GameObjects.Image; key: string}>();
  ...
  private drawGeneratedDirtPath(g: Phaser.GameObjects.Graphics, points: Vec2[], width = 34, alpha = .96, detailed = true): void {
    if (points.length < 2) return;
    const signature = JSON.stringify([points,width]);
    let cached = this.dirtPathImages.get(signature);
    if (!cached) {
      const margin=width, left=Math.min(...points.map(p=>p.x))-margin, top=Math.min(...points.map(p=>p.y))-margin;
      const w=Math.max(...points.map(p=>p.x))-left+margin,h=Math.max(...points.map(p=>p.y))-top+margin;
      const resolution=Math.min(2,2048/Math.max(w,h));
      const key=`deployment-dirt-${this.dirtPathSerial++}`;
      const texture=this.textures.createCanvas(key,Math.ceil(w*resolution),Math.ceil(h*resolution));
      if(!texture) return;
      const ctx=texture.context; ctx.scale(resolution,resolution);ctx.translate(-left,-top);
      ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);
      for(const p of points.slice(1))ctx.lineTo(p.x,p.y);
      // Paint the validated samples exactly; cosmetic splines must not cut obstacles.
    ...
    cached.image.setAlpha(alpha).setVisible(true);
  }
