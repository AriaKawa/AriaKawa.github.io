...
import { treeRetention, propRandom } from './SceneryDensity';
...
function generate(left: number, right: number, top: number, bottom: number, roads: Vec2[][], sites: ScenerySite[], isLand: (p: Vec2) => boolean): SceneryProp[] {
 const cell = 160;
      ...
      for (let y=top;y<=bottom;y++) for (let x=left;x<=right;x++) {
        ...
        const location=theaterLatLon(p);
        if(frame<=2 && propRandom('prop:'+x+':'+y,91)>=treeRetention(threatAt(location.lat,location.lon))) continue;
...
export class SceneryWorld {
 ...
 /** Stable roadside population sampled only within the active world window. */
 roadsideSpawns(center: Vec2, radius: number): Array<Vec2 & { id: string }> {
  const result: Array<Vec2 & { id: string }> = [];
 ...
 clear(a: Vec2, b: Vec2, radius: number): boolean {
  return this.clearBodies(a, b, radius, this.bodies(a, b, radius));
 }
 private clearBodies(a: Vec2, b: Vec2, radius: number, bodies: SceneryBody[]): boolean {
  const dx=b.x-a.x,dy=b.y-a.y,l=dx*dx+dy*dy;
  return !bodies.some(o => {
 ...
 move(a: Vec2, b: Vec2, radius: number): Vec2 {
  // Every binary-search segment lies inside this swept region. Query its bodies
  // once instead of rebuilding the same chunk/prop lists up to 21 times.
  const bodies = this.bodies(a,b,radius);
  if (this.clearBodies(a,b,radius,bodies)) return b;
  // Sweep the whole step, including high speed travel, to prevent tunneling.
  let low=0,high=1;
  for(let i=0;i<20;i++) { const t=(low+high)/2,p={x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t}; if(this.clearBodies(a,p,radius,bodies)) low=t; else high=t; }
 ...
 deploymentRoute(points: Vec2[], radius: number, scale = 1, variant = 0, independentLane = false): Vec2[] {
  const end = points.at(-1)!;
  if(this.blocked(end,radius+3)) throw new Error('Convoy site is obstructed');
  const random=(salt:number)=>hash(Math.round(end.x),Math.round(end.y),variant*137+salt);
  ...
  for(const road of independentLane ? [] : this.roads) for(let i=1;i<road.length;i++) {
   ...
   if(d<=700*scale) {
    ...
    const direction=(t>.5 ? -1 : 1)*(variant%2 ? -1 : 1);
    let remaining=(450+(Math.floor(variant/2)%4)*60)*scale, approach={...p}, cursor=direction>0?i:i-1;
  ...
  // Prefer a deliberate defense lane: climb away from the highway, dip,
  // sweep across the field, then round the final bend into the core.
  const choices=candidates.slice(0,12);
  if(choices.length) choices.push(...choices.splice(0,variant%choices.length));
  for(const {join,approach:start} of choices) {
   const across={x:join.x-start.x,y:join.y-start.y};
   const outward={x:end.x-join.x,y:end.y-join.y};
   const fieldWidth=Math.hypot(outward.x,outward.y),expansion=Math.max(1,240*scale/(fieldWidth||1));
   outward.x*=expansion;outward.y*=expansion;
   for(const width of [1,.75]) {
    const bend=.5+random(11)*.4,dip=.2+random(23)*.3;
    const controls=[[0,0],[.03+random(31)*.08,bend*width],[.2+random(47)*.1,(bend+.1)*width],[.42,dip*width],[.65+random(59)*.12,dip*width],[.9,.65+random(71)*.17],[1,1]];
    const waypoints=controls.map(([u,v],i)=>i===controls.length-1?{...end}:{x:start.x+across.x*u+outward.x*v,y:start.y+across.y*u+outward.y*v});
    try {
     const route=this.roadsideEntry(this.smooth(this.route(waypoints,radius+3),radius));
     const distance=length(route), direct=Math.hypot(start.x-end.x,start.y-end.y);
     if(distance>=Math.max(400*scale,direct*1.15) && distance<=Math.min(1450*scale,direct*2.4)
      && Math.hypot(route.at(-1)!.x-end.x,route.at(-1)!.y-end.y)<1) return route;
    } catch { /* Try a narrower bend or the next nearby highway entry. */ }
   }
  }
  // Try broad, rounded S bends before allowing a constrained straight fallback.
  for(const start of [...candidates.slice(0,6).map(c=>c.approach),points[0]]) {
   const dx=end.x-start.x,dy=end.y-start.y,direct=Math.hypot(dx,dy);
   for(const amplitude of [240,170,100]) for(const side of [1,-1]) {
    const controls=Array.from({length:17},(_,i)=>{const t=i/16,offset=Math.sin(t*Math.PI*2)*amplitude*scale*side;
     return i===16 ? {...end} : i===0 ? {...start} : {x:start.x+dx*t-dy/(direct||1)*offset,y:start.y+dy*t+dx/(direct||1)*offset};});
    try {const curve=this.roadsideEntry(this.smooth(this.route(controls,radius+3),radius));
     const chord=Math.hypot(curve[0].x-end.x,curve[0].y-end.y);
     if(length(curve)>=Math.max(400*scale,chord*1.12) && length(curve)<=1650*scale && Math.hypot(curve.at(-1)!.x-end.x,curve.at(-1)!.y-end.y)<1) return curve;
    } catch { /* Try the opposite bend or a smaller sweep with full clearance. */ }
   }
  }
