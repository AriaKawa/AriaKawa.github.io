...
export class GlobeDeployScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.network = this.registry.get("network") as GameClient; const savedPlanet = this.registry.get("deadroadPlanetId"); this.activePlanet = savedPlanet === MISSOURI_PLANET_ID ? savedPlanet : "planet1-current";
  ...
  private deploy(): void {
    ...
    this.registry.set("battleSeed", seed); const shell = document.querySelector<HTMLElement>("#globe-deploy")!; shell.classList.add("deploying"); this.showDeploymentLoading();
  ...
  private returnToMenu(): void { document.querySelector<HTMLElement>("#globe-deploy")!.hidden = true; this.game.canvas.style.opacity = "1"; this.scene.start("MenuScene"); }
