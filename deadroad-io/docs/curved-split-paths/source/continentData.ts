...
function curvedFieldRoute(lot: RoadsideLot, source: InfestationPointDefinition, seed: number, index: number): ContractRouteCandidate {
  const angle = seededUnit(seed, 'field-angle') * Math.PI * 2 + index * 2.3999632297;
  const length = 1000 + seededUnit(seed, `length-${index}`) * 120;
  const bend = (seededUnit(seed, `bend-${index}`) > .5 ? 1 : -1) * (200 + seededUnit(seed, `sweep-${index}`) * 90);
  const curved = Array.from({length:33}, (_, i) => {
    const t = 1-i/32, offset = Math.sin(t*Math.PI*2)*bend;
...
export function generateDefenseContractLayout(lot: RoadsideLot, difficulty: number, contractId: string, seed: number, routeField?: (points: Vec2[], lane: number) => Vec2[]): ContractLayout {
  ...
  const laneCount = difficulty >= 4.5 ? 2 : 1;
  for (const candidate of candidates) {
    try {
      const points = strategicField && routeField ? routeField(candidate.points, selected.length) : candidate.points;
      const distance = polylineLength(points);
      if (strategicField && (distance > 1650 || distance < 100)) continue;
      // Each front must demand its own coverage, not overlap the first lane.
      if (selected.some(existing => !points.some(p => Math.hypot(p.x-lot.x,p.y-lot.y)>220 &&
        existing.points.slice(1).every((b,i) => distanceToSegment(p,existing.points[i],b)>240)))) continue;
      selected.push({...candidate, points, distance});
  ...
  if (strategicField && selected.length < laneCount) throw new Error("Hard deployment needs two clear approaches. Move to more open ground.");
  ...
  const routeDanger = Math.max(...routes.map((entry) => entry.routeDanger));
  const padPools = routes.map(route => contractPads(contractId, lot, route, difficulty, seed));
  const buildPads: BuildPad[] = [];
  const padBudget = Math.max(8, Math.min(12, 12 - Math.floor(difficulty / 2)));
  for (let i=0;i<12 && buildPads.length<padBudget;i++) for (const pool of padPools) {
    const pad=pool[i];
    if (pad && buildPads.length<padBudget && buildPads.every(p=>Math.hypot(p.x-pad.x,p.y-pad.y)>=68))
      buildPads.push({...pad,id:`${contractId}-pad-${buildPads.length+1}`});
  }
  const layout = { spawnpoints, routes, buildPads, routeDanger };
