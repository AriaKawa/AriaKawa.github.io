...
export function queueCosmetics(scene: Phaser.Scene): void {
  for(const character of EXPEDITION_CHARACTERS)scene.load.spritesheet('expedition-'+character.id,`${ROOT}/${character.id==='axolotl'?'axolotl-classic-v3':character.id+'-expedition-v2'}.png`,{frameWidth:256,frameHeight:256});
