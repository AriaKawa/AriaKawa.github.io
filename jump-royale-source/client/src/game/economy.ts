...
export const price=(slot:string,id:string)=>id==='none'||(id==='original'&&slot!=='helmet')?0:slot==='character'||id==='party'?5:3;
export const owns=(slot:string,id:string)=>price(slot,id)===0||wallet().owned.includes(slot+':'+id);
...
export function playableOutfit(o:Outfit):Outfit {const result={...o};for(const slot of ['character','helmet','shirt','pants','hair'] as CosmeticSlot[]){if(!owns(slot,result[slot]))result[slot]=slot==='helmet'?'none':'original';}if(!owns('animalHat',result.animalHat??'none'))result.animalHat='none';return result;}
