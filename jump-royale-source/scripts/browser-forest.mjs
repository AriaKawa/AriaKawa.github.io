...
try{
 ...
 const png=await p.evaluate(async()=>{const {paintForestSky}=await import('/src/game/forestArt.ts');const c=document.createElement('canvas');c.width=1280;c.height=720;paintForestSky(c.getContext('2d'));return c.toDataURL().split(',')[1];});
 fs.mkdirSync('client/public/assets/menu/map-previews',{recursive:true});fs.writeFileSync('client/public/assets/menu/map-previews/forest.png',Buffer.from(png,'base64'));
 ...
 assert.deepEqual(errors,[]);console.log('PASS',initial,'keyboard jump, horizontal camera, responsive render, top-15 rewards, elimination/early leave give no point, replay, no browser errors');
