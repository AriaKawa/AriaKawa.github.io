...
export function stepGhosts(players:Iterable<PlayerState>,now:number,dt:number,bounds={left:SHAFT_LEFT,right:SHAFT_RIGHT,height:WORLD_HEIGHT}):void {
  for(const p of players) {
    ...
    if(p.isBot && !p.ghost && !p.departed && now-(p.eliminatedAt??now)>1200) {
      if((p.bot?.pattern??0)%5!==4)becomeGhost(p);else p.departed=true;
    }
    if(!p.ghost)continue;
    const dx=p.isBot?Math.sin(now/(1200+p.colorIndex%5*230)+p.colorIndex):Number(p.input.right)-Number(p.input.left);
    const dy=p.isBot?-.45+Math.sin(now/(1600+p.colorIndex%4*280)+p.colorIndex)*.65:Number(!!p.input.down)-Number(!!p.input.up);
    const speed=(p.isBot?100+p.colorIndex%6*17:440)/Math.max(1,Math.hypot(dx,dy));
