...
import { CURATED_USA_LAYER_NAME, curatedUSARoads } from "./CuratedUSARoadLayer";
...
export class RoadEditorController {
  ...
  destroy(): void { this.events.abort(); this.resizeObserver.disconnect(); window.clearTimeout(this.noticeTimer); if (this.dialog.open) this.dialog.close(); this.shell.hidden = true; }
  ...
  private pointerDown(event: PointerEvent): void {
    ...
    if (event.button === 1 || event.button === 2 || this.spaceHeld && event.button === 0) { this.panGesture = { clientX: event.clientX, clientY: event.clientY, panX: this.panX, panY: this.panY }; this.canvas.classList.add("panning"); return; }
  ...
  private pointerMove(event: PointerEvent): void {
    if (this.panGesture) { this.panX = this.panGesture.panX + event.clientX - this.panGesture.clientX; this.panY = this.panGesture.panY + event.clientY - this.panGesture.clientY; this.render(); return; }
  ...
  private pointerUp(event: PointerEvent): void { if (this.canvas.hasPointerCapture(event.pointerId)) this.canvas.releasePointerCapture(event.pointerId); if (this.dragPoint?.moved) { this.undoStack.push(this.dragPoint.before); if (this.undoStack.length > 100) this.undoStack.shift(); this.redoStack = []; this.updateUi(); this.setStatus(this.currentSnap ? `Control point connected by ${this.currentSnap.type} snap.` : "Control point moved."); } this.dragPoint = undefined; this.panGesture = undefined; this.canvas.classList.remove("panning"); this.render(); }
  private zoomAt(event: WheelEvent): void { event.preventDefault(); const before = this.screenToWorld(event.offsetX, event.offsetY); this.zoom = clamp(this.zoom * Math.exp(-event.deltaY * .0013), .0025, 1.7); this.panX = event.offsetX - before.x * this.zoom; this.panY = event.offsetY - before.y * this.zoom; this.render(); }
  ...
  private resize(): void { const rect = this.canvas.getBoundingClientRect(); const dpr = Math.min(window.devicePixelRatio || 1, 2); this.canvas.width = Math.max(1, Math.round(rect.width * dpr)); this.canvas.height = Math.max(1, Math.round(rect.height * dpr)); if (this.zoom === 1 && this.panX === 0 && this.panY === 0) { const width = EDITOR_INITIAL_VIEW.right - EDITOR_INITIAL_VIEW.left; const height = EDITOR_INITIAL_VIEW.bottom - EDITOR_INITIAL_VIEW.top; this.zoom = Math.min((rect.width - 54) / width, (rect.height - 54) / height); this.panX = (rect.width - width * this.zoom) / 2 - EDITOR_INITIAL_VIEW.left * this.zoom; this.panY = (rect.height - height * this.zoom) / 2 - EDITOR_INITIAL_VIEW.top * this.zoom; } this.render(); }
