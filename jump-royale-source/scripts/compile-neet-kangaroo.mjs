...
const sources={neet:"exec-07c1d408-31a6-4f2d-874d-187b149fbd48.png",kangaroo:"exec-39015d48-adb4-4511-a356-f4231075bc0d.png"};
...
for(const [id,file] of Object.entries(sources)){
 ...
 const scale=28/Math.max(...cells.map(c=>Math.max(c.width,c.height))),frames=[];
 for(const [f,c] of cells.entries()){
  const w=Math.round(c.width*scale),h=Math.round(c.height*scale);
  frames.push({input:await sharp(c.input).resize(w,h,{kernel:'nearest'}).png().toBuffer(),left:f*32+Math.floor((32-w)/2),top:31-h});
 }
 const out=`client/public/assets/reforged/cosmetics/${id}.png`;
 await sharp({create:{width:384,height:32,channels:4,background:'#00000000'}}).composite(frames).png().toFile(out);
 strips.push({input:await sharp(out).resize(1536,128,{kernel:'nearest'}).png().toBuffer(),left:0,top:(strips.length)*144});
 console.log(`Compiled ${id}: 12 poses, 32px foot-aligned frames`);
}
await sharp({create:{width:1536,height:288,channels:4,background:'#304958'}}).composite(strips).png().toFile('docs/neet-kangaroo-frames.png');
