...
export class LocalSimulation {
  ...
  constructor(name: string, private snapshotListener: (snapshot: Snapshot) => void, private noticeListener: (notice: Notice) => void, private managed = false) {
    ...
    if (!managed) { this.restoreWarState(); try { const saved = JSON.parse(localStorage.getItem('deadroad-progression-v1') || '{}'); this.player.xp = Math.max(0, Number(saved.xp) || 0); this.player.researchXp = Math.max(0, Number(saved.researchXp) || 0); this.player.research = saved.research || {}; this.player.turretRewards = saved.turretRewards || []; } catch {} } this.syncSectorState();
  ...
  private saveProgression(): void { if (this.managed) return; try { const { xp, researchXp, research, turretRewards } = this.player; localStorage.setItem('deadroad-progression-v1', JSON.stringify({ xp, researchXp, research, turretRewards })); } catch {} }
  ...
  private moveAmbient(zombie: InternalZombie, dt: number, now: number): void {
    ...
    if(zombie.chasing && d<42) {
      if(now>=(zombie.nextAttackAt??0) && now>=base.shieldEndsAt) {
        ...
        if(base.hp<=0) {this.notice('Your convoy was overrun.',true);this.removeBase(false);}
      }
      return;
    }
