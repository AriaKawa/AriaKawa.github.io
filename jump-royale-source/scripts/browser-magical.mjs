...
try{
 ...
 const initial=await p.evaluate(()=>{const s=window.__FORGE_DEV__.scene.getScene('Game'),h=s.client.hosted;clearInterval(h.loop);return {map:s.mapId,platforms:s.minimapPlatforms.length,width:s.world.width,height:s.world.height};});assert.equal(initial.map,'magical');assert.equal(initial.width,2880);
 await p.screenshot({path:'docs/magical-start.png'});
 ...
 await p.waitForTimeout(700);assert(await p.evaluate(()=>window.__FORGE_DEV__.scene.getScene('Game').cameras.main.scrollX>900));await p.screenshot({path:'docs/magical-crossing.png'});
 const cameraStable=await p.evaluate(async()=>{const s=window.__FORGE_DEV__.scene.getScene('Game'),h=s.client.hosted,me=h.players.get(h.localId);await new Promise(r=>setTimeout(r,1200));const positions=[];for(const facing of [1,-1,1]){me.facing=facing;h.tick();await new Promise(r=>setTimeout(r,500));positions.push(s.cameras.main.scrollX);}return {positions,variants:[...new Set([...s.platformEntities.values()].map(c=>c.list.find(o=>o.texture)?.texture.key))],water:s.flood.texture.key};});assert(Math.max(...cameraStable.positions)-Math.min(...cameraStable.positions)<.5,JSON.stringify(cameraStable));assert.equal(cameraStable.variants.length,3);assert.equal(cameraStable.water,'magical-ai-stardust-tide');
 await p.evaluate(()=>{const s=window.__FORGE_DEV__.scene.getScene('Game'),h=s.client.hosted,me=h.players.get(h.localId);const ledge=h.platforms.find(p=>p.y<2500&&p.y>2200);me.y=ledge.y-20;me.x=ledge.x+ledge.w/2-7;h.tick();});await p.waitForTimeout(700);assert(await p.evaluate(()=>window.__FORGE_DEV__.scene.getScene('Game').children.list.find(o=>o.texture?.key==='magical-ai-moon-palace').alpha>.95));await p.screenshot({path:'docs/magical-palace.png'});
 await p.setViewportSize({width:900,height:650});await p.waitForTimeout(500);await p.screenshot({path:'docs/magical-small.png'});
 ...
 await p.getByRole('button',{name:'Climb again',exact:true}).click();await p.waitForFunction(()=>window.__FORGE_DEV__.scene.getScene('Game').snapshot?.phase==='playing');assert.equal(await p.evaluate(()=>window.__FORGE_DEV__.scene.getScene('Game').mapId),'magical');
