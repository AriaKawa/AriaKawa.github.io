...
export function createSettings(parent:HTMLElement,onOpen?:()=>void,returnToMenu?:()=>void):()=>void {
 ...
 dialog.innerHTML='<header><h2>Settings</h2><button type="button" class="pixel-button" aria-label="Close settings">×</button></header><label>Music <output></output><input type="range" min="0" max="100" step="1" aria-label="Music volume"></label><label>Sound effects <output></output><input type="range" min="0" max="100" step="1" aria-label="Sound effects volume"></label><label class="names-setting"><span>Show player names</span><input type="checkbox" aria-label="Show player names"></label>';
 ...
 const escape=(event:KeyboardEvent)=>{if(event.code!=='Escape')return;event.preventDefault();event.stopImmediatePropagation();if(event.repeat)return;const fitting=document.querySelector<HTMLDialogElement>('.w2-dialog[open],.loot-dialog[open]');if(fitting){fitting.close();return;}if(dialog.open)close();else{document.querySelectorAll<HTMLDialogElement>('dialog[open]').forEach(d=>d.close());open();}};
