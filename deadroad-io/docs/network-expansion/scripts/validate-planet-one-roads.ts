...
assert(roads.length === 76);
assert.equal(new Set(roads.map(r => r.id)).size, roads.length);
assert(roads.every(r => r.source === "manual_editor" && !r.locked && r.visible && ["highway", "major_road", "rural_road", "dirt_road"].includes(r.roadClass)));
assert.deepEqual(validateRoads(roads), []);
const exported = createExport(roads, "2026-09-05T00:00:00.000Z");
assert.deepEqual(JSON.parse(JSON.stringify(parseRoadEditorExport(JSON.stringify(exported)).roads)), JSON.parse(JSON.stringify(roads)));
...
for (const name of ["stLouis", "kc", "chicago", "la", "atlanta", "newYork"] as const) {
  ...
  assert(node && node.edges.length >= (name === "la" ? 2 : 3) && node.edges.length <= 4, `${name} must be one simple 3–6 arm junction`);
  ...
  assert.equal(nearby.length, name === "la" ? 0 : 1, `${name} has extra junctions within the metro anchor area`);
}
