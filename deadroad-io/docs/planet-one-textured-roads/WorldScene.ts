...
const ZOMBIE_COLORS: Record<ZombieState["type"], number> = { shambler: 0x8ead5c, runner: 0xd6dc70, bloater: 0x9d6ab0, armored: 0x8a9997, brute: 0xd05b4c };
...
export class WorldScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.networkCleanup.push(this.network.on("snapshot", (snapshot) => { const previous = this.snapshot; const recovered = this.detectRecoveredEquipment(snapshot); this.captureCombatFeedback(previous, snapshot); this.snapshot = snapshot; this.ensureTheaterWorld(); this.captureDestroyedBases(previous, snapshot); const base = this.localBase(); const signature = `${base?.id ?? "none"}:${base?.worldX ?? "x"}:${base?.worldY ?? "y"}`; if (this.theaterWorld && signature !== this.lastBaseWorldSignature) { this.lastBaseWorldSignature = signature; this.drawWorld(); } this.syncZombieTargets(); this.refreshHud(); recovered.forEach((type) => this.flashRecoveredEquipment(type)); this.refreshZoomUi(); this.revealWorldWhenReady(); }));
  ...
  update(_time: number, deltaMs: number): void {
    ...
    this.expeditionScenery?.update(zoom, this.sceneryRoads, (this.theaterWorld?.roadPois ?? []).map(p => ({ ...theaterPoint(p.lat,p.lon), id:p.id, kind:p.kind })), p => this.theaterWorld ? this.sceneryLand.some(m => this.pointInPolygon(p,m.points)) : true, (this.snapshot?.bases ?? []).map(b => this.baseWorldPoint(b)));
    ...
    if (this.snapshot) {
      ...
      const liveIds = new Set(this.snapshot.zombies.map((zombie) => zombie.id));
      ...
      for (const [id, render] of this.renderedZombies) if (!liveIds.has(id)) this.retireZombieVisual(id, render);
  ...
  private handleWorldClick(x: number, y: number): void {
    ...
    if (tower) { this.network.upgrade(tower.id); this.showNotice(`Upgrade request sent for ${TOWER_INFO[tower.type].name}.`); return; }
    if (base) {
  ...
  private deployConvoy(): void { const base = this.localBase(); if (!base || base.status !== "packed") return; if (!this.world) return; const strategicOnLand = this.convoyStrategicOnLand(base); const assessment = assessDeployment(this.world, base, Date.now(), this.hordeActive(base), strategicOnLand); if (!assessment.valid) { this.showNotice(assessment.message, true); return; } this.openHudPanel("none"); this.network.deployConvoy(strategicOnLand); }
  ...
  private ensureTheaterWorld(): void {
    if (!this.world || !this.snapshot || this.theaterWorld) return; this.theaterWorld = this.isMissouriPlanet() ? createMissouriTileWorld(this.snapshot.lots) : createEarthGlobeWorld(this.world, this.snapshot.lots); this.sceneryRoads = (this.isMissouriPlanet() ? this.theaterWorld.roads : normalGameplayHighways(this.theaterWorld.roads)).map(r => r.pointLatLon.map(p => theaterPoint(p.lat,p.lon))); this.visibleHighwayLines = undefined; this.drawWorld(); this.refreshZoomUi();
  }
  ...
  private refreshUsaLod(camera: Phaser.Cameras.Scene2D.Camera, zoom: number): void {
    ...
    const quantize = this.zoomBand === "country" ? 16000 : this.zoomBand === "regional" ? 6000 : this.zoomBand === "city" ? 1800 : this.zoomBand === "local" ? 420 : 110; const centerX = camera.worldView.centerX / BATTLE_WORLD_SCALE; const centerY = camera.worldView.centerY / BATTLE_WORLD_SCALE; const signature = `${this.zoomBand}:${this.debugMode}:${Math.round(zoom * 100)}:${Math.round(centerX / quantize)}:${Math.round(centerY / quantize)}:${camera.width}:${camera.height}`; if (signature === this.lastLodSignature) return; this.lastLodSignature = signature;
    ...
    const visibleRoads = this.isMissouriPlanet() ? this.theaterWorld.roads : normalGameplayHighways(this.theaterWorld.roads);
  ...
  private refreshSurfaceTexture(camera: Phaser.Cameras.Scene2D.Camera, zoom: number): void {
    const progress = Phaser.Math.Clamp((zoom - .018) / .19, 0, 1); const alpha = progress * progress * (3 - 2 * progress); if (!this.theaterWorld || alpha <= .01) { this.surfaceChunks.forEach((chunk) => chunk.image.setVisible(false)); return; }
    ...
    const firstColumn = Math.max(0, Math.floor(view.left / chunkSize)); const lastColumn = Math.min(Math.ceil(THEATER_WIDTH / chunkSize) - 1, Math.floor(view.right / chunkSize)); const firstRow = Math.max(0, Math.floor(view.top / chunkSize)); const lastRow = Math.min(Math.ceil(THEATER_HEIGHT / chunkSize) - 1, Math.floor(view.bottom / chunkSize)); const signature = `${lod}:${firstColumn}:${lastColumn}:${firstRow}:${lastRow}:${Math.round(alpha * 20)}`; if (signature === this.lastSurfaceSignature) return; this.lastSurfaceSignature = signature; const active = new Set<string>(); this.surfaceUseCounter++; const refreshStarted = performance.now(); let created = 0;
    for (let row = firstRow; row <= lastRow; row++) for (let column = firstColumn; column <= lastColumn; column++) {
      const cacheKey = `${lod}:${column}:${row}`; active.add(cacheKey); let chunk = this.surfaceChunks.get(cacheKey); if (!chunk) { chunk = this.createSurfaceChunk(cacheKey, column * chunkSize, row * chunkSize, chunkSize, pixels, lod); this.surfaceChunks.set(cacheKey, chunk); created++; }
    ...
    this.surfaceChunks.forEach((chunk, key) => { if (!active.has(key)) chunk.image.setVisible(false); }); this.trimSurfaceChunks(active);
  }
  ...
  private destroySurfaceChunks(): void { for (const chunk of this.surfaceChunks.values()) { chunk.image.destroy(); this.textures.remove(chunk.textureKey); } this.surfaceChunks.clear(); this.surfacePatternCache.clear(); }
  ...
  private drawGeneratedDirtPath(g: Phaser.GameObjects.Graphics, points: Vec2[], width = 34, alpha = .96, detailed = true): void {
    if (points.length < 2) return;
    const shoulder = Math.max(2, width * .18); const cornerRadius = Math.max(4, width * .72);
    g.lineStyle(width + shoulder, 0x4d402d, alpha * .48); this.strokeRoundedPolyline(g, points, cornerRadius);
    g.lineStyle(width + shoulder * .48, 0x735b3a, alpha * .92); this.strokeRoundedPolyline(g, points, cornerRadius);
    g.lineStyle(width, 0x997447, alpha); this.strokeRoundedPolyline(g, points, cornerRadius);
    g.lineStyle(width * .72, 0xb18b59, alpha * .52); this.strokeRoundedPolyline(g, points, cornerRadius);
    if (!detailed || width < 16) return;
    for (const side of [-1, 1]) {
      const rut = this.offsetPolyline(points, width * .2 * side); g.lineStyle(Math.max(1.3, width * .055), 0x60492e, alpha * .48); this.strokeRoundedPolyline(g, rut, cornerRadius);
      const wornEdge = this.offsetPolyline(points, width * .39 * side); g.lineStyle(Math.max(1, width * .035), side < 0 ? 0xc09a66 : 0x5c482f, alpha * .32); this.strokeRoundedPolyline(g, wornEdge, cornerRadius);
    }
  }
  ...
  private ensureZombieSprite(id: string, type: ZombieState["type"]): void {
    if (this.zombieSprites.has(id)) return; const visual = ZOMBIE_VISUALS[type]; if (!this.textures.exists(visual.textureKey)) return;
    this.zombieSprites.set(id, this.add.image(0, 0, visual.textureKey).setOrigin(.5, .56).setDepth(ZOMBIE_SPRITE_DEPTH));
  }

  private retireZombieVisual(id: string, render: ZombieVisualState): void {
    ...
    if (contract && !reachedCore) { playBloodSplat(this.spatialAudioGainForSimulationPoint(render.x, render.y)); this.spawnBloodDecal(id, render); }
  ...
  private updateZombieSprites(): void {
    ...
    for (const [id, sprite] of this.zombieSprites) {
      ...
      const visual = ZOMBIE_VISUALS[render.type]; const step = Math.sin(render.walkPhase); const side = Math.sin(render.walkPhase * .5) * .65; const baseScale = visual.displaySize * BATTLE_WORLD_SCALE / Math.max(sprite.width, sprite.height); const squash = step * .025;
      sprite.setPosition((point.x + side) * BATTLE_WORLD_SCALE, (point.y + step * visual.bob) * BATTLE_WORLD_SCALE).setRotation(render.angle + Math.sin(render.walkPhase * .5) * visual.sway).setScale(baseScale * (1 - squash * .45), baseScale * (1 + squash)); visible++;
    }
  ...
  private drawDynamic(): void {
    ...
    if (!this.snapshot) return; const g = this.dynamicGraphics; const hud = this.zombieHudGraphics; const effects = this.projectileEffectGraphics; const now = Date.now(); const zoom = this.effectiveZoom(); g.clear(); hud.clear(); effects.clear();
    ...
    if (zoom >= .52) for (const zombie of this.snapshot.zombies) {
      const render = this.renderedZombies.get(zombie.id) ?? initialZombieVisual(zombie); const size = zombie.type === "brute" ? 30 : zombie.type === "bloater" ? 24 : zombie.type === "armored" ? 20 : zombie.type === "runner" ? 15 : 18;
  ...
  private drawTheaterDynamic(): void {
    if (!this.snapshot || !this.theaterWorld) return; const g = this.dynamicGraphics; const hud = this.zombieHudGraphics; const effects = this.projectileEffectGraphics; const territories = this.territoryOverlayGraphics; const now = Date.now(); const zoom = this.effectiveZoom(); const band = this.zoomBand; const player = this.localPlayer(); const base = this.localBase(); const visualScale = .55; g.clear().setPosition(0, 0); hud.clear().setPosition(0, 0); effects.clear().setPosition(0, 0); territories.clear().setPosition(0, 0);
    if (["city", "regional", "country"].includes(band)) for (const territory of earthTerritories) { const sector = this.snapshot.warSectors.find((entry) => entry.regionId === territory.id || entry.id === territory.id); const state = sector?.state === "secure" ? "safe" : sector?.state ?? (territory.infestation >= 72 ? "infested" : territory.infestation >= 38 ? "contested" : "safe"); const color = STATE_COLORS[state]; for (const points of tacticalTerritoryShapes(territory)) { if (!this.polygonInView(points, 6000)) continue; const alpha = state === "overrun" ? .28 : state === "infested" ? .17 : state === "contested" ? .1 : .045; territories.fillStyle(color, alpha).fillPoints(points, true); } }
    ...
    if (base) { const point = this.baseWorldPoint(base); if (this.pointInView(point.x, point.y, 2000)) this.drawBase(g, { ...base, coreX: point.x, coreY: point.y }, now); }
  ...
  private drawRoutePreview(g: Phaser.GameObjects.Graphics, points: Vec2[]): void { this.drawGeneratedDirtPath(g, points); }
  ...
  private drawContractPreview(g: Phaser.GameObjects.Graphics, contract: DefenseContract | undefined, lot: RoadsideLot, now: number): void {
    ...
    for (const route of contractRoutes) {
      if (!activeIds.has(route.id)) continue; const points = route.points.map((entry) => this.toTheater(entry)); this.drawRoutePreview(g, points);
  ...
  private drawTheaterMapOverlay(): void {
    ...
    for (const territory of earthTerritories) { const sector = this.snapshot?.warSectors.find((entry) => entry.id === territory.id || entry.regionId === territory.id); const state = sector?.state === "secure" ? "safe" : sector?.state ?? (territory.infestation >= 72 ? "infested" : territory.infestation >= 38 ? "contested" : "safe"); const color = { safe: "#72c7a1", contested: "#e2bd65", infested: "#e97845", overrun: "#a62f38", reclaiming: "#61d7db" }[state]; for (const ring of earthTerritoryBoundaryRings(territory)) { const points = ring.map(([lon, lat]) => theaterPoint(lat, lon)); ctx.beginPath(); points.forEach((point, index) => index ? ctx.lineTo(point.x * sx, point.y * sy) : ctx.moveTo(point.x * sx, point.y * sy)); ctx.closePath(); ctx.fillStyle = `${color}55`; ctx.fill(); ctx.strokeStyle = "#aeb19488"; ctx.lineWidth = .7; ctx.stroke(); } }
  ...
  private syncZombieTargets(): void { for (const zombie of this.snapshot?.zombies ?? []) if (!this.renderedZombies.has(zombie.id)) this.renderedZombies.set(zombie.id, initialZombieVisual(zombie)); }
