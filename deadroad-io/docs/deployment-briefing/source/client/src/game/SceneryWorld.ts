...
export class SceneryWorld {
 ...
 deploymentRoute(points: Vec2[], radius: number, scale = 1, variant = 0): Vec2[] {
  ...
  for(const {join,approach:start} of candidates.slice(0,6)) {
   const across={x:join.x-start.x,y:join.y-start.y};
   const outward={x:end.x-join.x,y:end.y-join.y};
   for(const width of [1,.75]) {
    const controls=[[0,0],[.04,.68*width],[.22,.78*width],[.40,.43*width],[.70,.43*width],[.94,.68],[1,1]];
    const waypoints=controls.map(([u,v])=>({x:start.x+across.x*u+outward.x*v,y:start.y+across.y*u+outward.y*v}));
    try {
