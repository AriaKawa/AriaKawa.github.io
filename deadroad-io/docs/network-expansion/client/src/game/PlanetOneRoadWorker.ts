  ...
  try {
    ...
    if (band === "local" || band === "base") {
      ...
      for (const shoulder of [true, false]) for (const road of sampled) {
        const width = road.roadClass === "dirt_road" ? band === "country" ? .45 : band === "regional" ? 1.2 : 2.5 : road.roadClass === "highway" ? band === "country" ? 1.8 : band === "regional" ? 4 : 8
        ...
        ctx.strokeStyle = road.roadClass === "dirt_road" ? shoulder ? "#62533d" : "#907552" : shoulder ? "#665f4d" : "#484943"; ctx.stroke();
      }
    }
