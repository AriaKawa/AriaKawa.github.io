...
export class NativeGameText {
  ...
  sync():void {
    ...
    for(const text of texts){
      ...
      const style=text.style,zoom=text.depth>=90?1:camera.zoom;
      const px=text.x-camera.scrollX*text.scrollFactorX,py=text.y-camera.scrollY*text.scrollFactorY;
      const x=(px-camera.width/2)*zoom+camera.width/2,y=(py-camera.height/2)*zoom+camera.height/2;
      ...
      Object.assign(label.style,{
        ...
        fontFamily:style.fontFamily,fontSize:`${parseFloat(String(style.fontSize))*sy*zoom}px`,
        fontWeight:style.fontStyle.includes('bold')?'700':'500',
        lineHeight:`${(parseFloat(String(style.fontSize))+text.lineSpacing+3)*sy*zoom}px`,
